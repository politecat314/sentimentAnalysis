...
""" fir_half = get_distance(apikey, customers[0]['origin'], couriers[0]['coordinate'])
sec_half =get_distance(apikey, couriers[0]['coordinate'], customers[0]['destination'])
...
""" 
for customer in customers:
    print('INFO plotting map for {cusName}...'.format(cusName = customer['name']), end='')
    for courier in couriers:
        enc_polyline = get_direction(apikey, customer['origin'], customer['destination'], courier['coordinate'])
        lat, long = zip(*polyline.decode(enc_polyline))
        gmap.plot(lat, long, color= courier['color'], edge_width =7)
    gmap.marker(get_lat(customer['origin']), get_long(customer['origin']), color='white', title=customer['name']+'Origin', label='C'+str(customers.index(customer)+1))
    gmap.marker(get_lat(customer['destination']), get_long(customer['destination']), color='red', title='Destination', label='*C'+str(customers.index(customer)+1))
    gmap.draw(customer['name']+'.html')
    print('Completed')
    print('INFO {}.html file created\n'.format(customer['name']))
...
polyline.draw('plotTest.html')
"""
""" 
def plot_polyline(oLat, oLong, dLat, dLong, hLat, hLong):
    polyline.directions(
        (oLat, oLong),
        (dLat, dLong),
        waypoints=[
    ...
    polyline.draw(customer['name']+'.html')

def get_hub_coordinate(name):
    for courier in couriers:
        if courier['name'] == name:
            return courier['coordinate']

for customer in customers:
    oLat = get_lat(customer['origin'])
    oLong = get_long(customer['origin'])
    dLat = get_lat(customer['destination'])
    dLong = get_lat(customer['destination'])
    hub = get_hub_coordinate(customer['courierRanking'][0]['name'])
    hLat = get_lat(hub)
    hLong = get_lat(hub)
    plot_polyline(oLat, oLong, dLat, dLong, hLat, hLong)
...
""" 
courier_rankings = [[]] * len(customers)

for customer in customers:
    for courier in couriers:
        print('RETRIEVING {0} distance for {1}...'.format(courier['name'], customer['name']), end='')
        fir_half = get_distance(apikey, customer['origin'], courier['coordinate'])
        sec_half =get_distance(apikey, courier['coordinate'], customer['destination'])
        total_distance = fir_half + sec_half
        new_dic = {
            'name': courier['name'],
            'total_distance': total_distance
        }
        courier_rankings[customers.index(customer)].append(new_dic)
        print('Completed\n') """
""" 
print('OUTPUT:')
for item in courier_rankings:
    for i in item:
        print(i)
"""


""" for courier_ranking in courier_rankings:
    for item in courier_ranking:
        n = len(item)
        quickSort(item, 0, n-1)
        print("Recommended couriers for Customer {} based on distance:".format(courier_ranking.index(item)+1))
        for i in range(n):
            print("{0}. {1} {2}".format(i+1, courier_ranking[i]['name'], courier_ranking[i]['total_distance'])), """

""" url1 = "https://maps.googleapis.com/maps/api/distancematrix/json?origins="

url2= "&destinations="

url3 = "&mode=driving&language=en&key="

for i in range(len(customers)):
    origin = customers[i]["origin"]
    ...
    for obj in output["rows"]:
        for data in obj["elements"]:
            print("Customer "+str(i+1)+" "+data["distance"]["text"])
"""


""" 
geolocator = Nominatim(user_agent="algorithm_group_project")

#list of hubs' coordinates
hubs = [
...
#Print address for each hub
for hub in hubs:
    location = geolocator.reverse(hub)
    print(location.address)  



gmap = gmplot.GoogleMapPlotter(3.0319924887507144, 101.37344116244806, 10, apikey=apikey)
